...
AI 오버레이(`F6`) 또는 인라인 AI 작업을 명시적으로 사용할 때만 전송됩니다. AI 요청이 없으면 어떤 데이터도 외부로 나가지 않습니다.

### 어떤 AI 프로바이더를 지원하나요?

OpenAI, Groq, `/v1` API를 제공하는 로컬 Ollama 또는 llama.cpp 서버 같은 OpenAI 호환 채팅 엔드포인트를 지원합니다. Anthropic은 OpenAI 호환 게이트웨이를 통해 사용할 수 있습니다. 프로바이더는 `~/.config/jvim/providers.toml`에 설정합니다.
...
API 키 또는 API 키 환경 변수 이름은 `~/.config/jvim/providers.toml`에 저장됩니다. 로그에 기록되거나 설정된 프로바이더 외부로 전송되지 않습니다.
...
`~/.config/jvim/providers.toml`을 열거나 생성한 뒤 OpenAI 호환 프로바이더를 추가하세요. [AI 오버레이 설정](/jvim-public/ko/usage/ai-overlay/)을 참고하세요.
