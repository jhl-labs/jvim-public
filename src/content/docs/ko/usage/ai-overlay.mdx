---
title: AI 오버레이
description: AI 프로바이더를 설정하면 에디터에서 Ask, Insert, Rewrite, 인라인 자동완성을 사용할 수 있습니다.
---

import AsciinemaPlayer from '../../../../components/AsciinemaPlayer.astro';
import KeymapTable from '../../../../components/KeymapTable.astro';

jvim의 AI 기능은 선택 사항입니다. 프로바이더를 설정한 뒤 AI 오버레이(`F6`)에서 Ask, Insert, Rewrite 모드를 사용할 수 있습니다. 원할 때 인라인 고스트 텍스트 자동완성과 AI 컨텍스트 메뉴도 켤 수 있습니다. 명시적으로 AI 기능을 호출하지 않는 한 네트워크 트래픽은 전송되지 않습니다.
...
**Ask**는 vault 문맥을 사용할 수 있는 Q&A 인터페이스입니다. 시맨틱 인덱스가 사용 가능한 경우, 첫 번째 응답은 답변 생성 전에 vault에서 관련 청크를 먼저 검색합니다.
...
jvim은 현재 OpenAI 호환 채팅 API에 연결합니다. OpenAI, Groq, `/v1` API를 제공하는 로컬 Ollama 또는 llama.cpp 서버를 사용할 수 있고, Anthropic은 OpenAI 호환 게이트웨이를 통해 사용할 수 있습니다.

프로바이더는 `~/.config/jvim/providers.toml`에 설정합니다:
...
로컬 LLM은 서버의 OpenAI 호환 엔드포인트를 가리키도록 설정하세요:
...
api_key_env = "OLLAMA_API_KEY" # 서버가 인증을 무시하면 임의 값
