...
`Ctrl+L`을 누르면 커밋 로그 오버레이가 열립니다 — 작성자, 짧은 SHA, 커밋 메시지와 함께 최근 커밋 목록이 표시됩니다. 히스토리를 검토하거나 최근 커밋이 반영되었는지 확인할 때 사용하세요.

![커밋 로그 오버레이](../../../../assets/images/46-commit-log-f5.png)

<KeymapTable rows={[
  { keys: 'Ctrl+L', action: '커밋 로그 열기', notes: '최근 커밋 목록: 작성자, SHA, 메시지' },
...
`F5`는 현재 저장소의 Git 패널을 엽니다. 여기에서 현재 파일의 hunk를 확인하고 공통 git 작업을 실행할 수 있습니다.

<KeymapTable rows={[
  { keys: 'F5', action: 'Git 패널 열기', notes: '현재 파일의 git 상태와 패널 작업 표시' },
  { keys: 'd', action: '현재 파일 diff 열기', notes: '활성 파일의 diff hunk 표시' },
  { keys: 'c', action: '현재 파일 커밋', notes: '메시지를 입력받고 활성 파일을 stage합니다' },
  { keys: 'p / P / f', action: 'push / pull / fetch', notes: '해당 system git 명령 실행' },
]} />
...
Git 작업은 Git 패널(`F5`)과 일부 커맨드 팔레트 작업(`F4` 또는 `Ctrl+P`)을 통해 사용할 수 있습니다. 작업 이름을 입력하면 필터링됩니다:

- **commit** — Git 패널을 엽니다. 패널에서 `c`를 눌러 현재 파일을 커밋합니다.
...
<KeymapTable rows={[
  { keys: 'F5', action: 'Git 패널 열기', notes: 'diff와 현재 파일 커밋 작업' },
  { keys: 'F4 / Ctrl+P', action: '커맨드 팔레트 열기', notes: 'git 작업 이름을 입력해 사용 가능한 작업 찾기' },
]} />
...
vault 검색 데이터베이스와 링크 메타데이터는 볼트를 인식하므로 `git checkout` 후에도 손상 없이 유지됩니다. 브랜치를 전환하고 볼트 내용이 크게 변경된 경우 수동 재인덱싱을 트리거하세요:

<KeymapTable rows={[
  { keys: 'Ctrl+R', action: '볼트 재인덱싱', notes: '풀텍스트 검색 DB와 링크 메타데이터를 재구성합니다' },
]} />
