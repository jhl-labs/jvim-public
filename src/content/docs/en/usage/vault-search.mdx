---
title: Vault Search
description: Full-text search across the open folder with Shift+F4, including tag and path filters.
---

import AsciinemaPlayer from '../../../../components/AsciinemaPlayer.astro';
import KeymapTable from '../../../../components/KeymapTable.astro';

jvim indexes the open markdown folder with SQLite FTS5 for practical full-text search. The search overlay supports plain queries, quoted phrases, and structured filters such as `tag:` and `path:`.
...
The search input understands these forms in addition to plain words:

- **`tag:<name>`** — matches files that contain the specified tag. For example, type `tag:travel` to find files tagged `#travel`.
- **`path:<glob>`** — limits results to matching paths. For example, `path:notes/*` searches files under `notes/`.
- **`"quoted phrase"`** — searches for an exact phrase.

You can combine filters with regular keywords in a single query: `tag:project meeting notes` finds files tagged `#project` whose body also contains the words "meeting" and "notes".
