---
title: Vault Search
description: Full-text search across the open folder with Shift+F4, including tag and wikilink tokens.
---

import AsciinemaPlayer from '../../../../components/AsciinemaPlayer.astro';
import KeymapTable from '../../../../components/KeymapTable.astro';

jvim indexes the open markdown folder with SQLite FTS5 for practical full-text search. The search overlay supports plain queries as well as `#tag` and `[[wikilink]]` tokens, so existing note conventions remain useful in the terminal.
